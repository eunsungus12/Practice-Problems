...
namespace CodingPractice.Sliding_Window
{
    ...
    class Longest_Continuous_Subarray_With_Absolute_Diff_Less_Than_or_Equal_to_Limit
    {
        public static int LongestSubarray(int[] nums, int limit)
        {
            ...
            {
                while (end < nums.Length && Math.Abs(nums[end] - runningMin) <= limit && Math.Abs(nums[end] - runningMax) <= limit)
                {
                    ...
                    if (end < nums.Length)
                    {
                        if (list.ContainsKey(nums[end])) list[nums[end]]++;
                        else list.Add(nums[end], 1);
                    }
                }
                counter = Math.Max(counter, end - start);
                if (list[nums[start]] > 1) list[nums[start]]--;
                else list.Remove(nums[start]);
                start++;
                runningMin = list.Count > 0 ? list.Keys[0] : 0;
                runningMax = list.Count > 0 ? list.Keys[list.Count - 1] : 0;
            }
            return counter;
        }
    ...
    //        {
    //            int runningMin = nums[i];
    //    int j = i + 1;
    //            while (j<nums.Length && Math.Abs(nums[j] - runningMin) <= limit)
    //            {
    //                runningMin = Math.Min(nums[j], runningMin);
    //                j++;
    //            }
